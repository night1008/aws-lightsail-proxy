locals {
  output_oss_object_key  = "outputs/anytls-configs/${var.config.region}/${var.config.instance_name}.json"
  local_output_file_name = "outputs/anytls-configs/${var.config.region}-${var.config.instance_name}.json"

  proxy_host_with_path = replace(replace(var.config.anytls_proxy_url, "https://", ""), "http://", "")
  sni                  = split("/", local.proxy_host_with_path)[0]

  anytls_port = var.config.anytls_port != null ? var.config.anytls_port : 8444
  ...
  # anytls://<password>@<host>:<port>?sni=<sni>&insecure=1#<tag>
  anytls_url = format(
